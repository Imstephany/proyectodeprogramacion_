from funciones_Main import *
consumo_energia = {
'Coca Codo Sinclair': {
'Quito': { 'consumos':(400, 432, 400, 432, 420, 432, 460, 432, 400, 432, 300 , 213),'tarifa': 65},
'Guayaquil': { 'consumos': (120, 55, 32, 120, 75, 32, 150, 55, 32, 120, 97, 32),'tarifa': 84},
},
'Sopladora': {
'Guayaquil':{ 'consumos': (310, 220, 321, 310, 220, 321, 310, 220, 321, 310, 220, 321),'tarifa':55},
'Quito': { 'consumos': (400, 432, 587, 400, 432, 587, 400, 432, 587, 400, 432, 587),'tarifa': 79},
'Loja': { 'consumos': (50, 32, 32, 50, 32, 32, 50, 32, 32, 50, 32, 32),'tarifa': 32}}}

informacion = {
'costa': ('Guayaquil', 'Manta'),
'sierra': ('Quito', 'Ambato', 'Loja'),
'oriente': ('Tena', 'Nueva Loja')
}

#Tema 1
planta,ciudad=comprobarEntrada(consumo_energia)
totalConsumo = tota_Megavatios(planta,ciudad,consumo_energia)
imprimirTema1(planta,ciudad,totalConsumo)

#Tema 2
ciudad = comprobarEntrada2(informacion)
dicPLantaConsumo = dic_plantasEnCiudad(consumo_energia,ciudad)
imprimirDicTema2(ciudad,dicPLantaConsumo)

#Tema 3
recaudado_en_region(consumo_energia,informacion)


#Tema 4
consumoPorMes(consumo_energia)

